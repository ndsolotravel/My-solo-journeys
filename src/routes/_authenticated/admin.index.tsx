  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        ...
        >
          <BarChart3 className="h-4 w-4" /> Live Analytics Dashboard{" "}
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Stat
          ...
          label="Comments"
          value={data?.comments}
          sub={data?.avgRating ? `★ ${data.avgRating} avg` : undefined}
          loading={isLoading}
        />
          ...
          {(data?.topPosts ?? []).map((p: any) => (
            <li key={p.id} className="flex items-center justify-between py-3 text-sm">
              <Link
                to="/blog/$slug"
                params={{ slug: p.slug }}
                className="hover:text-accent line-clamp-1"
              >
                {p.title}
              </Link>
