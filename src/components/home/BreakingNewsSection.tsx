    ...
    <>
      <div className={`mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8 ${className || ""}`}>
        ...
        >
          ...
          <div className="pointer-events-none absolute -right-12 -bottom-12 h-28 w-28 rounded-full bg-accent/20 blur-2xl opacity-50" />
            ...
            <div className="animate-ticker flex items-center shrink-0">
              {repeatedItems.map((item, idx) => (
                <div key={`t1-${item.id}-${idx}`} className="flex items-center">
                  ...
                  >
                    <span className="text-xs sm:text-sm font-medium text-neutral-100 group-hover/item:text-link-hover transition-colors tracking-normal">
                      {item.title}
                    </span>
        ...
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto p-6 sm:p-8 bg-background border border-border">
          {activeModalItem && (
            <div className="space-y-6">
              ...
              {/* Headline */}
              <h2 className="font-display text-2xl sm:text-3xl font-bold tracking-tight text-heading leading-snug">
                {activeModalItem.title}
              </h2>

              {/* Short Summary (if provided) */}
              {activeModalItem.summary && (
                <div className="rounded-2xl border-l-4 border-accent bg-accent/5 p-4 text-sm sm:text-base font-medium text-foreground/90 leading-relaxed">
                  {activeModalItem.summary}
                </div>
              )}
