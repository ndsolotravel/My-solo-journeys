...
export function DestinationsMap({ destinations }: { destinations: Destination[] }) {
  ...
  useEffect(() => {
    ...
    import("leaflet").then((LModule) => {
      ...
      // OpenStreetMap-based world street map with English-only labels (free, reliable, no API key, no watermarks)
      const tileLayer = L.tileLayer(
        "https://server.arcgisonline.com/ArcGIS/rest/services/World_Street_Map/MapServer/tile/{z}/{y}/{x}",
        {
          ...
          maxZoom: 19,
          attribution:
            'Tiles &copy; Esri &mdash; Sources: Esri, DeLorme, NAVTEQ, TomTom, USGS, &copy; <a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noopener noreferrer">OpenStreetMap</a> contributors',
        },
      );
      tileLayer.addTo(map);
      ...
      const pinIcon = L.divIcon({
        ...
        html: `
          <div style="position:relative;display:flex;align-items:center;justify-content:center;">
            <div style="position:absolute;width:24px;height:24px;border-radius:9999px;background:color-mix(in srgb, var(--accent, #4085FF) 30%, transparent);animation:ping 2s cubic-bezier(0,0,0.2,1) infinite;"></div>
            <div style="width:14px;height:14px;border-radius:9999px;background:var(--accent, #4085FF);border:2.5px solid #ffffff;box-shadow:0 2px 6px rgba(0,0,0,0.4);position:relative;z-index:2;"></div>
          </div>
        `,
        iconSize: [24, 24],
        iconAnchor: [12, 12],
      ...
      destinations.forEach((d) => {
        ...
        const popup = `
          <div style="min-width:190px;max-width:220px;font-family:Roboto,system-ui,-apple-system,sans-serif;padding:2px;">
            ...
            ${descHtml}
            <a href="/destinations/${d.slug}" style="color:#4085FF;font-size:12px;font-weight:500;text-decoration:none;display:inline-flex;align-items:center;gap:3px;">View Story →</a>
          </div>`;
