...
export function LanguageSelector() {
  ...
  // Auto-dismiss the error banner after a while.
  useEffect(() => {
    if (!error) return;
  ...
  return (
    <div className="relative flex items-center notranslate" translate="no" ref={menuRef}>
      <button
        type="button"
              ...
              <li key={l.code}>
                <button
                  ...
                  className={`flex w-full items-center justify-between px-3 py-2 text-sm hover:bg-muted ${
                    l.code === lang ? "text-accent font-medium" : "text-foreground"
                  }`}
                >
                  <span>{l.label}</span>
