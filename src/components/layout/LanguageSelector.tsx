  ...
  return (
    ...
    >
      <button
        ...
        className="inline-flex h-9 items-center gap-1.5 rounded-full border border-border/60 bg-transparent px-3 text-xs font-medium text-[#5ABCFF] hover:text-[#5ABCFF] focus:text-[#5ABCFF] focus-visible:text-[#5ABCFF] hover:bg-muted/60 transition-colors"
      >
        <Globe className="h-3.5 w-3.5 text-accent" />
        <span className="text-[#5ABCFF]">{current.nativeName}</span>
      </button>
      {menuOpen && (
        <div
          role="menu"
              ...
              <li key={l.code}>
                <button
                  ...
                  className={`flex w-full items-center justify-between px-3 py-2 text-sm hover:bg-muted focus:bg-muted ${
                    l.code === lang ? "text-[#5ABCFF] font-medium bg-muted/40" : "text-foreground hover:text-[#5ABCFF] focus:text-[#5ABCFF]"
                  }`}
                >
