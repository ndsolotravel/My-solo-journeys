...
const BASE_POST_COLUMNS = "id,title,slug,excerpt,content,cover_image,category,tags,featured,views,reading_minutes,published_at,created_at,author_name,author_image_url,location_name,latitude,longitude";
const LIST_BASE_COLUMNS = "id,title,slug,excerpt,cover_image,category,tags,featured,views,reading_minutes,published_at,created_at,author_name,author_image_url,location_name,latitude,longitude";
const FULL_POST_COLUMNS = "id,title,slug,excerpt,content,cover_image,category,tags,featured,views,reading_minutes,published_at,created_at,destination_id,travel_date,location_name,latitude,longitude,seo_title,seo_description,og_image_url,author_name,author_image_url,primary_keyword,secondary_keywords";
const LIST_POST_COLUMNS = "id,title,slug,excerpt,cover_image,category,tags,featured,views,reading_minutes,published_at,created_at,destination_id,travel_date,location_name,latitude,longitude,seo_title,seo_description,og_image_url,author_name,author_image_url,primary_keyword,secondary_keywords";
...
const listPosts = createServerFn({
  ...
  return fetchWithCache(cacheKey, 6e4, async () => {
    const {
      supabaseAdmin
    } = await import("./client.server-CVdEp4yu.mjs");
    let filterDestId = null;
    if (data.destination) {
      ...
      if (isUuid) {
        ...
        const {
          data: dRow
        } = await supabaseAdmin.from("destinations").select("id").eq("slug", destInput).maybeSingle();
        if (dRow?.id) {
    ...
    const buildQuery = (selectCols) => {
      ...
      if (data.categories && data.categories.length) {
        const catFilters = data.categories.flatMap((c) => [`category.ilike.${c}`, `tags.cs.{${c}}`, `tags.cs.{${c.toLowerCase()}}`]);
    ...
    const {
      resolveMediaUrl
    } = await import("./admin.functions-CnEC2dM5.mjs");
...
const getPostBySlug = createServerFn({
  ...
  return fetchWithCache(cacheKey, 6e4, async () => {
    ...
    let postRes = await supabaseAdmin.from("posts").select(`${FULL_POST_COLUMNS},destinations(id,title,slug),post_gallery(id,image_url,alt_text,sort_order),post_translations(language_code,title,excerpt,content,seo_title,seo_description)`).eq("slug", data.slug).eq("published", true).maybeSingle();
    if (postRes.error) {
    ...
    const {
      data: rawRelated
    } = await supabaseAdmin.from("posts").select(`${LIST_POST_COLUMNS},destinations(id,title,slug)`).eq("published", true).eq("category", post.category).neq("slug", data.slug).order("published_at", {
      ascending: false
    }).limit(3);
    ...
    try {
      void supabaseAdmin.from("posts").update({
    ...
    const fullPost = {
      ...
      author_image_url: post.author_image_url ? resolveMediaUrl(post.author_image_url, supabaseAdmin) : post.author_image_url,
      gallery
    };
    return {
      post: fullPost,
...
async function computeJourneyCountries(client) {
  return fetchWithCache("journey_countries", 6e4, async () => {
    const {
      data: posts
