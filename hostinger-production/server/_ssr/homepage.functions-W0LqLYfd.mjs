...
import { computeJourneyCountries } from "./posts.functions-Bt0QAIcW.mjs";
...
const HOMEPAGE_DEFAULTS = {
  ...
  homepage_hero_description: "Welcome to NDSOLOTRAVEL, a personal travel journal covering solo travel, motorcycle adventures, and mountain treks across Pakistan, the Karakoram, and around the world.",
...
async function resolveLatestPosts(client, limit = 3) {
  const {
    data: withCover,
    error: coverErr
  } = await client.from("posts").select(POST_COLS).eq("published", true).not("cover_image", "is", null).neq("cover_image", "").order("published_at", {
    ascending: false
  }).limit(limit);
  ...
  if (error || !Array.isArray(data)) {
    return Array.isArray(withCover) ? withCover.map((row) => ({
      ...row,
      cover_image: row.cover_image ? resolveMediaUrl(row.cover_image, client) : null
    })) : [];
  }
...
const getHomepageConfig = createServerFn({
  ...
  return fetchWithCache("homepage_config", 6e4, async () => {
    const {
      supabaseAdmin
    ...
    const stats = {
      countries: countriesMode === "manual" && settings.homepage_stat_countries.trim() !== "" ? manualCountries : journey.countriesCount,
...
const adminGetHomepageEditor = createServerFn({
  ...
  const {
    data: posts,
    error
  } = await client.from("posts").select("id,title,slug,featured,published,scheduled_at,published_at,created_at,cover_image").order("published_at", {
    ascending: false
  });
