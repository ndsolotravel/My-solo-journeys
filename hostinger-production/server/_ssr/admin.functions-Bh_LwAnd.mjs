...
import { e as extractBlogMediaPath, r as resolveMediaUrl } from "./media-DKXRUyGU.mjs";
import "../_libs/seroval.mjs";
import "../_libs/react.mjs";
import "../_libs/ws.mjs";
import { o as objectType, s as stringType, n as numberType, e as enumType, a as arrayType, b as booleanType, c as nullType, l as literalType } from "../_libs/zod.mjs";
...
async function resolveOrAssignDestination(client, params) {
  ...
  const {
    ...
    KNOWN_COUNTRY_HINTS
  } = await import("./posts.functions-S458rgU7.mjs");
  if (mode === "manual") {
...
const adminDetectDestination_createServerFn_handler = createServerRpc({
  id: "ee69daccd8f1283c990ab90c551dbf1cb33c40952f9e283ce796a3fadca4ffc9",
  name: "adminDetectDestination",
  filename: "src/lib/admin.functions.ts"
}, (opts) => adminDetectDestination.__executeServer(opts));
...
}).parse(i)).handler(adminDetectDestination_createServerFn_handler, async ({
  context,
  data
}) => {
  await assertEditor(context.userId, context.supabase);
...
const adminUpsertPost = createServerFn({
  ...
  if (data.category) {
    try {
      const {
        data: matchedCat
      } = await client.from("categories").select("id").ilike("name", data.category.trim()).maybeSingle();
      if (matchedCat?.id) {
  ...
  if (data.author_image_url !== void 0) {
...
const adminDeletePost = createServerFn({
  ...
  const {
    data: post,
    error: fetchError
  } = await client.from("posts").select("id, destination_id, cover_image, og_image_url, post_gallery(image_url)").eq("id", data.id).maybeSingle();
  if (fetchError) {
  ...
  if (post.destination_id) {
    try {
      const {
        data: remaining
      } = await client.from("posts").select("id").eq("destination_id", post.destination_id);
      if (!remaining || remaining.length === 0) {
        const {
          data: dest
        } = await client.from("destinations").select("id, category").eq("id", post.destination_id).maybeSingle();
...
const adminListDestinations = createServerFn({
  ...
  const {
    data,
    error
  } = await client.from("destinations").select("*, posts:posts(id, title, slug, published)").order("created_at", {
    ascending: false
  });
  if (error) throw new Error(error.message);
  return (data ?? []).map((d) => ({
...
const adminUpsertDestination = createServerFn({
  method: "POST"
}).middleware([requireSupabaseAuth]).validator((i) => destInputSchema.parse(i)).handler(adminUpsertDestination_createServerFn_handler, async ({
  context,
  data
}) => {
  await assertEditor(context.userId, context.supabase);
  ...
  const payload = {
    ...
    longitude: typeof data.longitude === "number" && !isNaN(data.longitude) ? data.longitude : null
  };
  if (data.id) {
    const {
      data: updated,
      error
    } = await client.from("destinations").update(payload).eq("id", data.id).select("*").single();
    if (error) throw new Error(error.message);
    return {
      ok: true,
      destination: updated
    ...
    const {
      data: inserted,
      error
    } = await client.from("destinations").insert(payload).select("*").single();
    if (error) throw new Error(error.message);
    return {
      ok: true,
...
const adminDeleteDestination = createServerFn({
  ...
  const {
    data: linkedPosts
  } = await client.from("posts").select("id, title").eq("destination_id", data.id);
  if (linkedPosts && linkedPosts.length > 0) {
    throw new Error(`Cannot delete destination: ${linkedPosts.length} story(ies) are currently assigned to this destination. Please reassign or unlink the stories before deleting.`);
  }
  const {
    data: deleted,
    error
...
const adminUpdateDestinationCoordinates_createServerFn_handler = createServerRpc({
  id: "4037c4fa29b33ee2d3d1feacec22fd0de3eded8bdb36fc0638b5eab1a997adbd",
  name: "adminUpdateDestinationCoordinates",
  filename: "src/lib/admin.functions.ts"
}, (opts) => adminUpdateDestinationCoordinates.__executeServer(opts));
const adminUpdateDestinationCoordinates = createServerFn({
...
}).parse(i)).handler(adminUpdateDestinationCoordinates_createServerFn_handler, async ({
  context,
  data
}) => {
  await assertEditor(context.userId, context.supabase);
  ...
  const {
    data: updated,
    error
  } = await client.from("destinations").update({
    latitude: data.latitude,
    longitude: data.longitude
...
const adminUpdatePostCoordinates_createServerFn_handler = createServerRpc({
  id: "b99fcd4d4a84f8e2f7edaaf7b978a348d2c99f98ed8abcb4ad716b549cb9d742",
  name: "adminUpdatePostCoordinates",
  filename: "src/lib/admin.functions.ts"
}, (opts) => adminUpdatePostCoordinates.__executeServer(opts));
const adminUpdatePostCoordinates = createServerFn({
...
}).parse(i)).handler(adminUpdatePostCoordinates_createServerFn_handler, async ({
  context,
  data
}) => {
  await assertEditor(context.userId, context.supabase);
  ...
  const {
    data: updated,
    error
  } = await client.from("posts").update(payload).eq("id", data.id).select("id, latitude, longitude, location_name").single();
  if (error) throw new Error(error.message);
  return {
    ok: true,
    post: updated
...
const adminAnalytics = createServerFn({
  ...
  const [posts, comments, subs, msgs, top] = await Promise.all([client.from("posts").select("id,published,scheduled_at,views", {
    ...
    count: "exact",
    head: false
  }), client.from("subscribers").select("id,status", {
    count: "exact"
...
const adminCreateAdminUser_createServerFn_handler = createServerRpc({
  id: "ffadc0811b3c311e38a0c7f23a5dfb1b1778216f1639c80b541652ff3643bed2",
  name: "adminCreateAdminUser",
  filename: "src/lib/admin.functions.ts"
}, (opts) => adminCreateAdminUser.__executeServer(opts));
const adminCreateAdminUser = createServerFn({
...
}).handler(adminCreateAdminUser_createServerFn_handler, async ({
  context,
  data
}) => {
  await assertEditor(context.userId, context.supabase);
  ...
  if (process.env.SUPABASE_SERVICE_ROLE_KEY) {
    ...
    const url = process.env.NEXT_PUBLIC_SUPABASE_URL || process.env.SUPABASE_URL || "https://mqoybarqgzzvillignbr.supabase.co";
    const anonKey = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || process.env.SUPABASE_ANON_KEY || "sb_publishable_quAPYI3nYdGK50erwAPnfg_YJWBq2u5";
...
const adminListStaffUsers_createServerFn_handler = createServerRpc({
  id: "93b11c49e51b9e7b809eeaf78d25b25dc2804f4931c5d974ae247b05eeacb608",
  name: "adminListStaffUsers",
  filename: "src/lib/admin.functions.ts"
}, (opts) => adminListStaffUsers.__executeServer(opts));
const adminListStaffUsers = createServerFn({
  method: "GET"
}).middleware([requireSupabaseAuth]).handler(adminListStaffUsers_createServerFn_handler, async ({
  context
}) => {
  await assertEditor(context.userId, context.supabase);
  const {
    data: roles,
    error
  } = await context.supabase.from("user_roles").select("id, user_id, role, created_at");
  if (error) {
  ...
  const {
    data: profiles
  } = await context.supabase.from("profiles").select("id, username");
...
export {
  ...
  adminCreateAdminUser_createServerFn_handler,
  ...
  adminDetectDestination_createServerFn_handler,
  ...
  adminListStaffUsers_createServerFn_handler,
  adminSavePostGallery_createServerFn_handler,
  adminTogglePublish_createServerFn_handler,
  adminUpdateDestinationCoordinates_createServerFn_handler,
  adminUpdatePostCoordinates_createServerFn_handler,
