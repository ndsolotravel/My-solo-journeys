...
import { e as extractBlogMediaPath, r as resolveMediaUrl } from "./media-DUkNwMwq.mjs";
...
const POST_COLS = "id,title,slug,excerpt,content,cover_image,category,tags,featured,published,published_at,scheduled_at,reading_minutes,views,created_at,updated_at,destination_id,travel_date,location_name,latitude,longitude,seo_title,seo_description,og_image_url,author_name,author_image_url";
...
const adminListMessages = createServerFn({
  ...
  const {
    data: msgData,
    error: msgError
  } = await client.from("messages").select("id,name,email,subject,message,status,is_read,created_at,updated_at").order("created_at", {
    ascending: false
  }).limit(500);
...
const adminUpdateMessageStatus = createServerFn({
  ...
  await assertEditor(context.userId, context.supabase);
  const client = context.supabase ?? (await import("./client.server-Dg1wI_zl.mjs")).supabaseAdmin;
  const status = data.status || (typeof data.is_read === "boolean" ? data.is_read ? "read" : "new" : "read");
  const {
    error: msgErr
