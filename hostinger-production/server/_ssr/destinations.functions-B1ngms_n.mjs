...
const listDestinations = createServerFn({
  ...
  const {
    data: allPosts
  } = await supabaseAdmin.from("posts").select("id, title, slug, cover_image, destination_id, published, published_at, created_at").eq("published", true).order("published_at", {
    ascending: false,
    nullsFirst: false
  });
  const resolved = (data ?? []).map((row) => {
    const linkedPosts = (row.posts ?? []).filter((p) => p.published !== false && p.cover_image);
    ...
    if (!coverPhoto && allPosts) {
      const matchedPost = allPosts.find((p) => p.destination_id === row.id || p.slug && row.slug && (p.slug.includes(row.slug) || row.slug.includes(p.slug)) || p.title && row.title && (p.title.toLowerCase().includes(row.title.toLowerCase()) || row.title.toLowerCase().includes(p.title.toLowerCase())));
