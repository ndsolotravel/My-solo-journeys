...
const adminListMessages = createServerFn({
  ...
  await assertEditor(context.userId, context.supabase);
  const client = context.supabase ?? (await import("./client.server-DgxrFlIS.mjs")).supabaseAdmin;
  let res = await client.from("messages").select("id,name,email,subject,message,is_read,created_at").order("created_at", {
    ascending: false
  }).limit(500);
  if (res.error) {
    console.warn("[adminListMessages] Querying messages failed, checking fallback contact_messages:", res.error.message);
    res = await client.from("contact_messages").select("id,name,email,subject,message,status,created_at").order("created_at", {
      ascending: false
    }).limit(500);
    if (res.error) throw new Error(res.error.message);
