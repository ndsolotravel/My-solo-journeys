...
const TOPIC_CLUSTERS = [
  {
    ...
    description: "A complete guide to solo motorcycle adventures across Pakistan — from the Karakoram Highway to Deosai Plains, Hunza Valley, Skardu, and Fairy Meadows.",
...
- **Fuel:** Fill up at every opportunity — stations are sparse in remote areas`
  },
  {
    ...
    description: "From Nanga Parbat base camp to Baltoro Glacier, explore the best trekking routes in Pakistan's Karakoram, Himalaya, and Hindu Kush ranges.",
...
- **Winter treks** are possible but require serious preparation`
  },
  {
    ...
    description: "Explore the Karakoram range — home to K2, the Baltoro Glacier, and some of the most dramatic mountain scenery in the world.",
...
The Karakoram is accessible from May to October. July and August offer the warmest weather but also the most crowds. June and September are ideal for fewer people and clear skies.`
  },
  {
    ...
    description: "Everything you need to know about travelling in Gilgit Baltistan — from Hunza and Skardu to the Karakoram Highway and remote valleys.",
...
Most of Gilgit Baltistan is open to foreign tourists without a special permit. However, areas near the Line of Control (like Minimarg and some parts of Baltistan) may require a NOC.`
  },
  {
    ...
    description: "Is Pakistan safe for solo travellers? Everything you need to know about solo travel in Pakistan — routes, tips, costs, and real experiences.",
...
Pakistan is one of the most affordable travel destinations in the world. Budget travellers can get by on $20-30 per day, including accommodation, food, and transport.`
  },
  {
    ...
    description: "The complete Pakistan travel guide — visa requirements, best time to visit, top destinations, budget tips, and safety advice for first-time visitors.",
...
const listActiveTopics = createServerFn({
  ...
  try {
    ...
    const {
      resolveMediaUrl
    } = await import("./admin.functions-CnC9mk6Y.mjs").then((n) => n.y);
    const {
      data: postsData,
      error: postErr
    } = await supabaseAdmin.from("posts").select("id,title,slug,cover_image,category,tags,destination_id").eq("published", true);
    if (postErr) {
...
const getTopicCluster = createServerFn({
  ...
  try {
    ...
    let query = supabaseAdmin.from("posts").select("id,title,slug,excerpt,cover_image,category,tags,featured,views,reading_minutes,published_at,created_at,author_name,location_name,seo_title,seo_description,og_image_url,destinations(title,slug)").eq("published", true);
    const tagFilters = topic.tags.flatMap((tag) => [`tags.cs.{${tag}}`, `tags.cs.{${tag.toLowerCase()}}`, `tags.cs.{${tag.charAt(0).toUpperCase() + tag.slice(1)}}`]);
    const catFilters = topic.categories.flatMap((cat) => [`category.eq.${cat}`, `category.eq.${cat.toLowerCase()}`, `category.eq.${cat.charAt(0).toUpperCase() + cat.slice(1)}`]);
    const uniqueFilters = Array.from(/* @__PURE__ */ new Set([...tagFilters, ...catFilters]));
