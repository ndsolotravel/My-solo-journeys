...
const typographySchema = objectType({
  headingFont: stringType().min(1),
  bodyFont: stringType().min(1),
  navigationFont: stringType().min(1),
  buttonFont: stringType().min(1),
  headingWeight: stringType().min(1),
  bodyWeight: stringType().min(1),
  bodySize: objectType({
    desktop: numberType().min(12).max(28),
    tablet: numberType().min(12).max(26),
    mobile: numberType().min(11).max(24)
  }),
  lineHeight: objectType({
    desktop: numberType().min(1.1).max(2.4),
    tablet: numberType().min(1.1).max(2.4),
    mobile: numberType().min(1.1).max(2.4)
  }),
  letterSpacing: objectType({
    desktop: numberType().min(-0.1).max(0.2),
    tablet: numberType().min(-0.1).max(0.2),
    mobile: numberType().min(-0.1).max(0.2)
  }),
  headingLetterSpacing: numberType().min(-0.1).max(0.2).default(-0.02),
  // Independent Script Font validation
  scriptFont: stringType().min(1).default("Yuyu Short"),
  scriptWeight: stringType().min(1).default("400"),
  scriptSize: objectType({
    desktop: numberType().min(12).max(48),
    tablet: numberType().min(12).max(42),
    mobile: numberType().min(11).max(36)
  }).default({ desktop: 26, tablet: 22, mobile: 19 }),
  scriptLineHeight: objectType({
    desktop: numberType().min(0.9).max(2.4),
    tablet: numberType().min(0.9).max(2.4),
    mobile: numberType().min(0.9).max(2.4)
  }).default({ desktop: 1.3, tablet: 1.25, mobile: 1.2 }),
  scriptLetterSpacing: objectType({
    desktop: numberType().min(-0.1).max(0.2),
    tablet: numberType().min(-0.1).max(0.2),
    mobile: numberType().min(-0.1).max(0.2)
  }).default({ desktop: 0.01, tablet: 5e-3, mobile: 0 })
});
function parseTypographyConfig(raw) {
  ...
  try {
    ...
    if (typeof parsed === "object" && parsed !== null) {
      ...
      return {
        ...
        headingLetterSpacing: Number(obj.headingLetterSpacing) !== void 0 && !isNaN(Number(obj.headingLetterSpacing)) ? Number(obj.headingLetterSpacing) : DEFAULT_TYPOGRAPHY_CONFIG.headingLetterSpacing,
        // Script font merge
        ...
        scriptLetterSpacing: {
          desktop: Number(obj.scriptLetterSpacing?.desktop) !== void 0 && !isNaN(Number(obj.scriptLetterSpacing?.desktop)) ? Number(obj.scriptLetterSpacing?.desktop) : DEFAULT_TYPOGRAPHY_CONFIG.scriptLetterSpacing.desktop,
          tablet: Number(obj.scriptLetterSpacing?.tablet) !== void 0 && !isNaN(Number(obj.scriptLetterSpacing?.tablet)) ? Number(obj.scriptLetterSpacing?.tablet) : DEFAULT_TYPOGRAPHY_CONFIG.scriptLetterSpacing.tablet,
          mobile: Number(obj.scriptLetterSpacing?.mobile) !== void 0 && !isNaN(Number(obj.scriptLetterSpacing?.mobile)) ? Number(obj.scriptLetterSpacing?.mobile) : DEFAULT_TYPOGRAPHY_CONFIG.scriptLetterSpacing.mobile
        }
      };
    }
...
function resolveGoogleFamily(fontName) {
  const foundScript = CURATED_SCRIPT_FONTS.find(
    (f) => f.name.toLowerCase() === fontName.toLowerCase() || f.googleFamily.toLowerCase() === fontName.toLowerCase()
  );
  if (foundScript) {
...
function generateGoogleFontsUrl(config) {
  const fontSlotMap = /* @__PURE__ */ new Map();
  const addFont = (name, requestedWeights) => {
    if (!name) return;
    const resolved = resolveGoogleFamily(name);
    const existing = fontSlotMap.get(resolved.family) || /* @__PURE__ */ new Set();
...
function generateTypographyCss(config, selector = ":root") {
...
export {
  CURATED_SCRIPT_FONTS as C,
  DEFAULT_TYPOGRAPHY_CONFIG as D,
  generateTypographyCss as a,
  CURATED_GOOGLE_FONTS as b,
  generateGoogleFontsUrl as g,
  parseTypographyConfig as p,
  resolveGoogleFamily as r,
  typographySchema as t
};
