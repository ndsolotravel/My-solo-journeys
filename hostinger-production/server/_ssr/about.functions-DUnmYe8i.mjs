...
const ABOUT_DEFAULTS = {
  ...
  about_seo_description: "Meet Hussain, the engineer and solo explorer behind NDSOLOTRAVEL. Documenting raw motorcycle expeditions, high-altitude Himalayan treks, and stories across 27 countries.",
  about_og_title: "About Hussain & NDSOLOTRAVEL — Solo Adventure Travel",
  about_og_description: "Solo motorcycle journeys, high-altitude trekking, and honest field notes from 27 countries and 3 continents.",
  about_og_image: "",
...
const adminGetAboutEditor_createServerFn_handler = createServerRpc({
  id: "14a5f2da7be60dd7371c795a0d155a8d64fe7f0f70cfc6b669d6c624dd941cd1",
  name: "adminGetAboutEditor",
  filename: "src/lib/about.functions.ts"
}, (opts) => adminGetAboutEditor.__executeServer(opts));
const adminGetAboutEditor = createServerFn({
  method: "GET"
}).middleware([requireSupabaseAuth]).handler(adminGetAboutEditor_createServerFn_handler, async ({
  context
}) => {
  ...
  const {
    data,
    error
  } = await client.from("site_settings").select("key, value").in("key", ABOUT_KEYS);
  if (error) {
  ...
  if (Array.isArray(data)) {
    for (const row of data) {
      if (row.value !== void 0 && row.value !== null && row.value !== "") {
...
const adminSaveAboutSettings_createServerFn_handler = createServerRpc({
  id: "46e91631002081989b72bdedfecbf910ff9f8a3dd454b304205425730e6377da",
  name: "adminSaveAboutSettings",
  filename: "src/lib/about.functions.ts"
}, (opts) => adminSaveAboutSettings.__executeServer(opts));
const adminSaveAboutSettings = createServerFn({
...
}).handler(adminSaveAboutSettings_createServerFn_handler, async ({
  data,
  context
}) => {
  await assertEditor(context.userId, context.supabase);
  const client = context.supabase ?? (await import("./client.server-CVdEp4yu.mjs")).supabaseAdmin;
  const upserts = Object.entries(data.settings).filter(([key]) => ABOUT_KEYS.includes(key)).map(([key, value]) => ({
    key,
    value: value ?? "",
  ...
  const {
    error
  } = await client.from("site_settings").upsert(upserts, {
    onConflict: "key"
  });
  if (error) {
...
export {
  adminGetAboutEditor_createServerFn_handler,
  adminSaveAboutSettings_createServerFn_handler
};
