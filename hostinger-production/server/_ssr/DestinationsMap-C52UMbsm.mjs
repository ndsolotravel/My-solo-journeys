...
function coordsFor(d) {
  const dLat = typeof d.latitude === "number" ? d.latitude : Number(d.latitude);
  const dLng = typeof d.longitude === "number" ? d.longitude : Number(d.longitude);
  if (!isNaN(dLat) && !isNaN(dLng) && dLat >= -90 && dLat <= 90 && dLng >= -180 && dLng <= 180 && (d.latitude != null || d.latitude != null)) {
    return [dLat, dLng];
  }
