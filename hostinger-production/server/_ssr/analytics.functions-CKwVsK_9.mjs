...
const recordPageViewAndPing = createServerFn({
  ...
  let rawCountry = null;
  try {
    rawCountry = getRequestHeader("cf-ipcountry") || getRequestHeader("x-country-code") || getRequestHeader("x-vercel-ip-country") || null;
  } catch {
  }
...
const getAdminAnalyticsDetails = createServerFn({
  ...
  const [liveCountRes, totalSessionsRes, totalViewsRes, todaySessionsRes, todayViewsRes, yesterdaySessionsRes, periodPageViewsRes, periodSessionsRes, allSessionsRes, allPageViewsRes, topPostsRes, recentVisitorsRes] = await Promise.all([
    ...
    supabaseAdmin.from("visitor_sessions").select("session_id", {
      count: "exact",
      head: true
    ...
    // 3. Total Page Views
    supabaseAdmin.from("page_views").select("id", {
      count: "exact",
      head: true
    }),
    // 4. Today Visitors
    ...
    supabaseAdmin.from("page_views").select("created_at, path, title").gte("created_at", periodStart.toISOString()).order("created_at", {
      ascending: true
    }),
    // 8. Sessions in period
    supabaseAdmin.from("visitor_sessions").select("session_id, created_at, device_type, referrer_source, country").gte("created_at", periodStart.toISOString()).order("created_at", {
      ascending: true
    }),
    // 9. Device & Source distribution from visitor_sessions
    supabaseAdmin.from("visitor_sessions").select("device_type, referrer_source, country").limit(2e3),
    // 10. Popular pages overall
    supabaseAdmin.from("page_views").select("path, title").limit(5e3),
    // 11. Top Blog Posts
    supabaseAdmin.from("posts").select("id, title, slug, category, cover_image, views").eq("published", true).order("views", {
      ascending: false
    }).limit(5),
    // 12. Recent Visitors Log
    supabaseAdmin.from("visitor_sessions").select("session_id, country, device_type, browser, os, entry_page, last_active_at").order("last_active_at", {
      ascending: false
    }).limit(15)
  ]);
