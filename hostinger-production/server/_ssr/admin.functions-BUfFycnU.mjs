...
const adminListPosts = createServerFn({
  ...
  const {
    data: fullData,
    error: fullError
  } = await supabaseAdmin.from("posts").select(`${POST_COLS},destinations(title,slug)`).order("updated_at", {
    ascending: false
  });
  ...
  const {
    data: baseData,
    error: baseError
  } = await supabaseAdmin.from("posts").select(BASE_POST_COLS).order("updated_at", {
    ascending: false
  });
...
const adminGetPost = createServerFn({
  ...
  const {
    data: fullData,
    error: fullError
  } = await supabaseAdmin.from("posts").select(`${POST_COLS},destinations(id,title,slug),post_gallery(id,image_url,alt_text,sort_order)`).eq("id", data.id).maybeSingle();
  let row = fullData;
  if (fullError || !row) {
    const {
      data: baseData,
      error: baseError
    } = await supabaseAdmin.from("posts").select(BASE_POST_COLS).eq("id", data.id).maybeSingle();
    if (baseError) throw new Error(baseError.message);
    row = baseData;
  }
  if (!row) return null;
